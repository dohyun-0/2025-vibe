...
def load_data():
    df_total = pd.read_csv(
        "202506_202506_연령별인구현황_월간_합계.csv", encoding="cp949"
    )
    df_gender = pd.read_csv(
        "202506_202506_연령별인구현황_월간_남녀구분.csv", encoding="cp949"
    )
    df_econ = pd.read_csv(
        "연령별_경제활동인구_총괄_20250725132144.csv", encoding="utf-8"
    )
...
def preprocess(df):
    ...
    df["연령구간인구수"] = (
        df["2025년06월_계_연령구간인구수"].str.replace(",", "").astype(int)
    )
...
def preprocess_gender(df):
    ...
    df["시도"] = df["행정구역"].apply(lambda x: x.split()[0])
    drop_cols = [
        col for col in df.columns if "총인구수" in col or "연령구간인구수" in col
    ]
    ...
    for col in cols:
        try:
            ...
            for _, row in df.iterrows():
                age_group = row["연령계층별"]
                if pd.isna(age_group) or age_group in [
                    "15세 이상 전체",
                    "15 - 29세",
                    "15 - 64세",
                ]:
...
def preprocess_total_avg(df):
    ...
    for (year, metric), vals in data_dict.items():
        result.append(
            {"연도": year, "지표": metric, "값": round(sum(vals) / len(vals), 2)}
        )
...
def main():
    ...
    with tab1:
        ...
        df_city = (
            df_total.groupby("시도")[["총인구수", "연령구간인구수"]].sum().reset_index()
        )
        option = st.radio(
            "표시할 항목", ["총인구수", "연령구간인구수"], horizontal=True
        )
        fig = px.bar(
            df_city,
            x="시도",
    ...
    with tab2:
        ...
        selected_region = st.selectbox(
            "행정구역 선택", df_total["행정구역"].unique(), key="age_tab"
        )
    ...
    with tab3:
        ...
        selected_region = st.selectbox(
            "행정구역 선택", df_gender["행정구역"].unique(), key="gender_tab"
        )
        ...
        df_melted = df_gender_plot.melt(
            id_vars="연령", var_name="성별", value_name="인구수"
        )
        fig3 = px.line(
            df_melted,
            x="연령",
            ...
            color="성별",
            markers=True,
            title=f"{selected_region} 연령별 남녀 인구 분포",
            template="plotly_dark",
        )
    ...
    with tab4:
        ...
        fig4 = px.line(
            ...
            color="연령계층",
            markers=True,
            title=f"연도별 {selected_type} (연령계층별)",
            template="plotly_dark",
        )
